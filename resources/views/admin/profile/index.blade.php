...
@section('content')
    <div class="card">
        ...
        <div class="row">
            <div class="col-md-4 col-12 p-5">
                @if (isset($profile))
                    @if ($profile->img == Null)
                        <img src="{{ asset('img/profile.jpg') }}" alt="Profile Image Placeholder" class="rounded-circle img-fluid item-hover" id="editProfilePic" data-id="{{$profile->id}}">    
                    @else
    ...
    @include('admin.profile.partials.profile-pic_modal')
    @include('admin.profile.partials.signature_modal')
